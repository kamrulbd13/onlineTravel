...
{
    return view('admin.faqs.visa-add');
}
public function allVisa()
{
    $data['allVisa']=Visa::get();
    return view('admin.faqs.all-visa',$data);
}
public function deleteVisa($id)
    {
        $flight = Visa::findOrFail($id);
        $flight->delete();
        $this->setSuccessMessage('Request Deleted!! ');
        return redirect()->back();
...
public function postVisa(Request $request){
    $request->validate([
        'country_name'=>'required|unique:visas,country_name',
        'country_details'=>'required',
        'country_image'=>'required|mimes:jpeg,png,jpg,webp,svg|max:1114',
        'visa_details'=> 'required',
    ]);

    try {if($request->hasfile('country_image'))
            {
                $input= new Visa;
                $input->country_name=$request->country_name;
                $input->country_details=$request->country_details;
                $input->visa_details=$request->visa_details;

                $file=$request->file('country_image');
                $extenstion = $file->getClientOriginalExtension();
                $filename=time().'.'.$extenstion;
                $file->move('uplods/visa/',$filename);
                $input->country_image=$filename;
            }
            $input->save();
            $this->setSuccessMessage('New Visa Added!!!');
            return redirect()->back();
